...
Begin VB.Form frmT46V207A 
   ...
   Begin MSComctlLib.Toolbar Toolbar 
      ...
      Begin VB.Frame fraCondicaoInspGeral 
         Appearance      =   0  'Flat
         BackColor       =   &H80000004&
         Caption         =   "Condição de Inspeção para todos os itens:"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
...
Private Sub ConfigGrid()
    With flxSolicInspecao
        ...
        .ColWidth(9) = 1000
        .TextMatrix(0, 10) = "Insp obrigatória"
        .ColWidth(10) = 1200
        .Col = 0
    End With
End Sub
...
Private Function mfBuscaInspecao() As Boolean
    ...
    For Each mobjstcPedLoc In mobjstcPed.ColecaoPedLoc
        ...
        lSelect = lSelect & " and num_cnpj_cpf = " & mobjstcPed.Num_Cgc_Cpf
    ...
    If mobjstcInspecao Is Nothing Then
        MsgBox "Não foram localizadas Inspeções anteriores"
    Else
...
Dim PedLoc As stcA46V708B
    ...
    If MsgBox("Deseja selecionar esta opção para todos os itens?", vbQuestion + vbYesNo) = vbYes Then
        Screen.MousePointer = vbHourglass
        For Each PedLoc In mobjstcPed.ColecaoPedLoc
