...
Begin VB.Form frmT46V003A 
   ...
   Caption         =   "Pesquisa cotação/proposta/apólice/protocolo"
   ClientHeight    =   5520
   ClientLeft      =   2655
   ClientTop       =   2475
   ClientWidth     =   10395
   ...
   Begin Threed.SSFrame fraX 
      ...
      Begin Threed.SSFrame fraSitDocumento 
         ...
         Begin VB.OptionButton optSitDocumento 
            Caption         =   "Inspeções não Solicitadas"
            ForeColor       =   &H00800000&
            Height          =   400
            Index           =   3
            Left            =   4515
         ...
         End
         Begin VB.OptionButton optSitDocumento 
            Caption         =   "Apenas propostas não transmitidas"
            ForeColor       =   &H00800000&
            Height          =   400
            Index           =   1
            Left            =   1200
   ...
   Begin MSComctlLib.Toolbar barFerramentas 
      ...
      BeginProperty Buttons {66833FE8-8583-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Button6 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Key             =   "Limpar"
            Object.ToolTipText     =   "Limpar critérios de pesquisa."
            ImageIndex      =   4
         EndProperty
   ...
   Begin VB.Label lblObservacao 
      AutoSize        =   -1  'True
      Caption         =   "* Proposta já transmitida."
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Menu mnuEditar 
      ...
      Begin VB.Menu mnuEdiLimpar 
         Caption         =   "&Limpar critérios de pesquisa"
      End
...
Private mNum_Apolice As String    'Número da apólice selecionado.
Private mNum_Item As String    'Número do item selecionado.
Private mPesqApolice As Boolean    'Indica se a tela está preparada para pesquisar apólice ou não.
'Somente será "true" se for a pesquisa para a tela de renovação.
Private mRegChecked As String    'Registros selecionados para exclusão (armazena "Nosso Numero").
...
'Procedure: habilita controles
    ...
    '1. Opção de selecionar - HABILITAR.
    mnuArqSelecionar.Enabled = True
    barFerramentas.Buttons(4).Enabled = True    'Selecionar.

    '2. Opção de copiar documento selecionado - HABILITAR APENAS PARA COTAÇÕES E PROPOSTAS.
    If optDocumento(0) = True Then      'Cotações e propostas.
        mnuEdiCopiar.Enabled = True
        barFerramentas.Buttons(7).Enabled = True    'Copiar documento selecionado.
    End If

    '3. Opção de excluir documento selecionado - HABILITAR PARA COTAÇÕES E PROPOSTAS e PROTOCOLOS.
...
Private Sub mpMenNaoEncontrado()
    ...
    If gM46V999.gfPreenchido(txtNumPedido.Text) Then
        lMensagem = lMensagem & vbLf & "Número do pedido = " & txtNumPedido.Text & "."
    Else
        If optDocumento(0) = True Then
            ...
            Else
                ...
                Else
                    ...
                    Else
                        lMensagem = lMensagem & " (apenas cotações sem solicitação de agM46V999.gEndamento)."
                    End If
                End If
            End If
        Else
        ...
        If gM46V999.gfPreenchido(txtIniVigencia.Text) And gM46V999.gfPreenchido(txtTerVigencia.Text) Then
            lMensagem = lMensagem & vbLf & "Vigência: de " & txtIniVigencia.Text & " a " & _
                        txtTerVigencia.Text & "."
        Else
            If gM46V999.gfPreenchido(txtIniVigencia.Text) Then
                lMensagem = lMensagem & vbLf & "Vigência em " & txtIniVigencia.Text & "."
            Else
                If gM46V999.gfPreenchido(txtTerVigencia.Text) Then
                    lMensagem = lMensagem & vbLf & "Vigência em " & txtTerVigencia.Text & "."
                End If
            End If
        End If
...
'Procedure: obtém Ramos.
    ...
    If lrsTbm_Ramos.EOF = True Then
        Call gM46V999.gpFecha2(lbdTbm_Ramos)
        Call gM46V999.gpGraLog(1, "FNL0055 - " & gM46V999.gPrefixo & "Tabela de Ramos não localizado.")
        Exit Sub
    End If
...
'Procedure: pesquisa cotações e propostas.
    ...
    Dim lrst As clsYasRecordSet    'Registro de apólice.
    ...
    With flxResultado
        ...
        .TextMatrix(0, 1) = "Ramo"
        .TextMatrix(0, 2) = "Apólice"
        .TextMatrix(0, 3) = "Segurado"
        .TextMatrix(0, 4) = "CNPJ/CPF"
        .TextMatrix(0, 5) = "Início Vigência"
        .TextMatrix(0, 6) = "Término Vigência"
        .TextMatrix(0, 7) = "Nosso número"
        .TextMatrix(0, 8) = "Situação do documento"
    ...
    Do While Not lrst.EOF
        ...
        If lrst("Cod_Ramo") = e_CodRamo_Condominio Then
            flxResultado.TextMatrix(lCount, 1) = "Condomínio"
        ...
        Else
            flxResultado.TextMatrix(lCount, 1) = "Não definido"
        End If
...
'Procedure: pesquisa cotações e propostas.
    ...
    If optSitDocumento(1) = True Then       'Propostas não transmitidas.
    ...
    With flxResultado
        'a) Posiciona quantidade de colunas e de linhas (apenas cabeçalho).
        .Cols = 12  '07/01/2010 - Ficha: 0901900 - Cópia de cotação/proposta c/ última tarifa
        .Rows = 1
        'b) Posiciona linha de cabeçalho (com altura).
        .Row = 0
        'c) Posiciona cabeçalho nas colunas.
        ...
        .TextMatrix(0, 7) = "Nosso número"
        .TextMatrix(0, 8) = "Situação do documento"
        .TextMatrix(0, 9) = "Expiração"
        .TextMatrix(0, 10) = "Ramo"    '07/01/2010 - Ficha: 0901900 - Cópia de cotação/proposta c/ última tarifa
            ...
            For Each lobjstcPedLoc In lObjstcPed.ColecaoPedLoc
                If lobjstcPedLoc.Insp_Obr = True And lobjstcPedLoc.Insp_Agend = False And lObjstcPed.DAT_DIGIT > 0 Then
                    If DateDiff("D", gM46V999.gfForData(2, lObjstcPed.DAT_DIGIT), Date) <= 35 Then
                        ...
                        flxResultado.TextMatrix(lCount, 10) = lObjstcPed.Cod_Ramo  '07/01/2010 - Ficha: 0901900 - Cópia de cotação/proposta c/ última tarifa
                        ...
                        If lObjstcPed.Dat_Pri_Calculo <> 0 And (gM46V999.gTipUsuario = e_Tipo_Usu_Corretor Or gM46V999.gTipUsuario = e_Tipo_Usu_CorretorYasuda) Then
                            ...
                            End If
                        End If
                        If lObjstcPed.Cod_Sit_Ped = e_SitPedido_Transmitido Then          'Já transmitido.
                            flxResultado.Row = lCount
                            lColuna = flxResultado.Col
                            For lI = 0 To (flxResultado.Cols - 1)
                                flxResultado.Col = lI
    ...
    Else
        For Each lObjstcPed In lObjPed.Colecao
            ...
            '1100773 - Sinalização Consulta de Propostas e Cotações - Correção
    ...
    flxResultado.Refresh
    '8. Mostra a caixa de seleção para exclusão multipla.
...
'Procedure: pesquisa protocolo de transmissão.
    ...
    With flxResultado
        ...
        .TextMatrix(0, 3) = "Início de vigência"
        .TextMatrix(0, 4) = "Término de vigência"
        .TextMatrix(0, 5) = "Tipo logradouro"
        .TextMatrix(0, 6) = "Logradouro"
        .TextMatrix(0, 7) = "Número logradouro."
        .TextMatrix(0, 8) = "Complemento"
        .TextMatrix(0, 9) = "Bairro"
        .TextMatrix(0, 10) = "Cidade"
        .TextMatrix(0, 11) = "UF"
        .TextMatrix(0, 12) = "CEP"
        .TextMatrix(0, 13) = "Nosso número"
        'd) Posiciona largura das colunas.
        .ColWidth(0) = 1620
        .ColWidth(1) = 1000
        .ColWidth(2) = 1500
...
'Exibe a coluna para marcar registros a serem excluidos.
    ...
    If chkExcluir.Value = 1 Then
        '1- Mostra a caixa de seleção utilizada para marcar todos s registros
        chkTodos.Visible = True
        chkTodos.Value = 0
        '2- Exibe a coluna utilizada para exclusão
        With flxResultado
            .ColWidth(0) = 700
        End With
        '3-
        ...
        ' Opção de copiar documento selecionado.
        mnuEdiCopiar.Enabled = False
        barFerramentas.Buttons(7).Enabled = False   'Copiar documento selecionado.
        '4- Desmarca todos os registros
        Call mpAlteraTodos(False)
        '5- Chama a função para remover a cor verde da linha selecionada
        flxResultado.CellBackColor = vbWhite
        Call flxResultado_SelChange
    Else
        '1- Oculta a caixa de seleção utilizada para marcar todos s registros
        chkTodos.Visible = False
        chkTodos.Value = 0
        '2- Exibe a coluna utilizada para exclusão
        With flxResultado
...
Private Sub flxResultado_Click()
    ...
    Dim lSelecao As String    'Seleção.

'1. Somente quando está selecionada a opção "Marcar para Excluir" e o tipo da consulta é
'   "Cotações e Propostas" - para executar o item 2 quando clicado em qualquer coluna da grade.
    If mTipDocumento = 1 And chkExcluir.Visible = True And chkExcluir.Value = 1 Then
        'Altera posição da coluna.
        flxResultado.Col = 0
    Else
        Exit Sub
    End If
    ...
    With flxResultado
        ...
        If flxResultado.Col = 0 Then
            If flxResultado.CellPicture = picChecked Then
                ...
                .Col = .Col + 7       'Coluna de nosso número.
                lSelecao = .Text
                'Quando desmarcar um registro, remove o número de pedido da lista.
                mRegChecked = Replace(mRegChecked, lSelecao & ",", "")
                'Antes de utilizar o campo no comando SQL, não esquecer de remover a última vírgula
                'contida na variável.
            Else
...
'Somente quando está selecionada a opção "Marcar para Excluir".
    If mTipDocumento = 1 And chkExcluir.Visible = True And chkExcluir.Value = 1 Then
        'Está halibitado para selecionar arquivos para excluir - sai do procedimento.
        Exit Sub
    End If
...
Private Sub flxResultado_SelChange()
    ...
    With flxResultado
        ...
        '2. Se linha já está com fundo verde, nada a fazer.
        ...
        'Somente quando esta selecionado a opção "Marcar para Excluir"
...
Private Sub mpPesquisaDocumento()
    ...
    '   Se foco está em algum campo que possue evento LostFocus, o mesmo deve ser acionado.
    '   Observação: é necessário verificar todos os campos que possuem evento LOSTFOCUS.
    ...
    If optDocumento(0) = False And optDocumento(1) = False And optDocumento(2) = False Then
        Call gM46V999.gpGraLog(1, "FNP0013 - Tipo de documento não selecionado.")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    ...
    If txtNumPedido.Visible = True Then
        If gM46V999.gfPreenchido(txtNumPedido.Text) Then
            If Not IsNumeric(txtNumPedido.Text) Then
                Call gM46V999.gpGraLog(1, "FGE0080 - Número do pedido não numérico.")
                Screen.MousePointer = vbDefault
                Exit Sub
            End If
            lTemNumPedido = True
    ...
    If gM46V999.gTipUsuario = e_Tipo_Usu_Yasuda Then
        If gM46V999.gfPreenchido(txtSegurado.Text) = False And gM46V999.gfPreenchido(txtNumPedido.Text) = False Then
            If cboRamo.ListIndex = 0 Then
                Call gM46V999.gpGraLog(1, "FRA0001 - Ramo não selecionado.")
                Screen.MousePointer = vbDefault
                Exit Sub
            End If
        End If
        ...
        If txtIniVigencia.Visible = True Then
            If gM46V999.gfPreenchido(txtIniVigencia.Text) Then
                If gM46V999.gfVerData(2, txtIniVigencia.Text) = False Then
                    Call gM46V999.gpGraLog(1, "FGE0018 - Vigência inválida (data de início de vigência inválida).")
        ...
        If txtTerVigencia.Visible = True Then
            If gM46V999.gfPreenchido(txtTerVigencia.Text) Then
                If gM46V999.gfVerData(2, txtTerVigencia.Text) = False Then
                    Call gM46V999.gpGraLog(1, "FGE0081 - Vigência inválida (data de término de vigência " & _
                                              "inválida).")
        ...
        If txtIniVigencia.Visible = True And txtTerVigencia.Visible = True And _
           gM46V999.gfPreenchido(txtIniVigencia.Text) And gM46V999.gfPreenchido(txtTerVigencia.Text) Then
            If gM46V999.gfForData(1, txtIniVigencia.Text) > gM46V999.gfForData(1, txtTerVigencia.Text) Then
                Call gM46V999.gpGraLog(1, "FGE0082 - Vigência inválida (data de início de vigência " & _
                                          txtIniVigencia.Text & ") maior que a data de término de vigência (" & _
                                          txtTerVigencia.Text & ").")
                If txtIniVigencia.Visible = True And txtIniVigencia.Enabled = True Then
                    txtIniVigencia.SetFocus
                End If
...
Private Sub mnuArqSelecionar_Click()
    ...
    Dim lNumPedido As String    'Número de pedido.
    Dim lSitDocumento As Byte   'Situação do documento.
    ...
    With flxResultado
        ...
        '    Exit Sub
        'End If
        'Caso cotação expirada exibe mensagem com opção para exclusão
        If mTipDocumento = 1 Then
            If .Cols > 11 Then
                If .TextMatrix(.Row, 11) = 1 Then
                    If .TextMatrix(.Row, 9) = 1 Then
                        lMensagem = "AGE0046 - Cotação " & lNumPedido & " (nosso número " & _
                                    lNosNumero & ") expirou o prazo de validade, e não será exibida. Deseja excluir este item da lista?"
        ...
        If mPesqApolice = True Then
            'Se for pesquisa de apólice , deverá então retornar a apólice para a tela de origem
            mNum_Apolice = .TextMatrix(.Row, 2)      'Número da apólice.
            mNum_Item = 0
        ...
        Else
            Select Case mTipDocumento
            Case 1      'Cotação ou proposta - grava registro de comunicação.
                lNumPedido = .TextMatrix(.Row, 2)       'Número do pedido.
                lNosNumero = .TextMatrix(.Row, 7)       'Nosso número.
                lSitDocumento = .TextMatrix(.Row, 8)    'Situação do documento.
                If lSitDocumento = 3 Then               'Já transmitido.
                    lMensagem = "AGE0019 - Proposta " & lNumPedido & " (nosso número " & _
                                lNosNumero & ") já transmitida. Confirma abertura desta proposta?"
                    If MsgBox(lMensagem, vbQuestion + vbYesNo + vbDefaultButton2) = vbYes Then
                        Call gM46V999.gpGraLog(0, lMensagem & " SIM.")
                    Else
                        Call gM46V999.gpGraLog(0, lMensagem & " NÃO.")
                ...
                End If
                Me.Hide
            Case 2      'Apólice - cria renovação de apólice.
                lNumApolice = .TextMatrix(.Row, 2)     'Número da apólice.
                lNumIteApolice = 0
                lMensagem = ""
                ...
                End If
                Me.Hide
            Case 3      'Protocolo de transmissão - exibe informações do protocolo.
                gM46V111.gNumPedido = .TextMatrix(.Row, 1)   'Número do pedido.
                gM46V111.gComNosNumero = .TextMatrix(.Row, 13)  'Nosso número.
                frmT46V006A.Show 1
            Case Else
...
Private Sub mnuEdiCopiar_Click()
    Dim lAntNosNumero As String     'Anterior: nosso número.
    Dim lAntNumPedido As String     'Anterior: número do pedido.
    Dim lMensagem As String    'Mensagem.
    Dim lNovNosNumero As String     'Novo: nosso número.
    ...
    With flxResultado
        ...
        lAntNumPedido = .TextMatrix(.Row, 2)    'Número do pedido.
        lAntNosNumero = .TextMatrix(.Row, 7)   'Nosso número.

        '3. Confirma com usuário.
        lMensagem = "AGE0009 - " & gM46V999.gPrefixo & "Confirma cópia deste documento?"
        ...
        If .TextMatrix(.Row, 8) = "3" Then    'Proposta transmitida
            'Verifico se é a cópia de uma proposta cancelada
...
Reentrada:
                        lMensagem = "Esta é uma proposta transmitida. Trata-se de uma reentrada de proposta?"
        ...
        lInputBox = InputBox("Digite a tarifa que será usada na copia (em branco usará a tarifa da original): ", Me.Caption)
        ...
        If lInputBox <> Empty Then
            If Not IsNumeric(lInputBox) Then
                Call gM46V999.gpGraLog(1, "AGE0002 - Tarifa inválida.")
                Screen.MousePointer = vbDefault
                Exit Sub
            Else
        ...
        'g) Conclusão.
        Call gM46V999.gpGraLog(1, "AGE0002 - Processo concluído: cópia realizada para pedido " & lNovNumPedido & _
                                " (nosso número " & lNovNosNumero & ").")
...
'Procedure: exclui documento selecionada (grava N no campo Cod_Ativo).
    ...
    Dim objTransm As clsA46V718A    'Objeto da classe clsA46V718A (TAB_PED_TRANSM).
    Dim objstcTransm As stcA46V718B   'Objeto da classe de estrutura stcA46V718B (TAB_PED_TRANSM).
    Dim lNumPedido As Long        'Número do pedido.
    Dim lNosNumero As String      'Nosso número.
...
'1- Verifica se existe lista de arquivos para excluir
    ...
    If gM46V999.gfPreenchido(mRegChecked) = True Then
        ...
        If mTipDocumento <> 1 Then
            Call gM46V999.gpGraLog(1, "FGE0083 - Operação de exclusão inválida para este tipo de documento.")
            'Limpa lista de arquivos selecionados para exclusão
            Call mpAlteraTodos(False)
            Exit Sub
        End If
        ...
        lMensagem = "AGE0023 - Confirma exclusão de " & lTotalExcluir & _
                  " registro(s)?"
        If MsgBox(lMensagem, vbYesNo + vbQuestion + vbDefaultButton2) = vbNo Then
            Call gM46V999.gpGraLog(0, lMensagem & " NÃO.")
            Exit Sub
        ...
        'Finalização OK
        Call mpDesControles
        Call gM46V999.gpGraLog(1, "AGE0002 - Processo concluído: " & lTotalExcluir & _
                                " registro(s) excluído(s) com sucesso.")

    Else
        '2- Exclusão de unico arquivo
        With flxResultado
            ...
            '2. Posiciona controles.
            Select Case mTipDocumento
            Case 1      'Cotação ou proposta.
                lNumPedido = .TextMatrix(.Row, 2)       'Número do pedido - quando tipo de Doc = 1.
                lNosNumero = .TextMatrix(.Row, 7)      'Nosso número.
            Case 3      'Protocolo.
                lNumPedido = .TextMatrix(.Row, 1)               'Número do pedido.
                lNosNumero = .TextMatrix(.Row, 13)       'Nosso número.
            Case Else
                lNosNumero = String(20, "0")
            End Select

            '3. Confirma com usuário.
            lMensagem = "AGE0023 - Confirma exclusão?" & vbLf & vbLf & "Documento " & lNumPedido & _
                      " com nosso número " & lNosNumero & "?"
            If MsgBox(lMensagem, vbYesNo + vbQuestion + vbDefaultButton2) = vbNo Then
                Call gM46V999.gpGraLog(0, lMensagem & " NÃO.")
                Exit Sub
            End If
            ...
            Case 1      'Cotação ou proposta.
                ...
                If UCase(objstcPedido.Cod_User_Liber) <> UCase(gM46V999.gNomUsuario6) Then
                    lMensagem = "AGE0023 - Confirma exclusão?" & vbLf & vbLf & "Documento " & _
                ...
                Call gM46V999.gpGraLog(1, "AGE0002 - Processo concluído: documento " & lNumPedido & _
...
'Desabilita os controles
    ...
    Case 2      'Protocolo de transmissão.
        mTipDocumento = 3
        lblNumPedido.Visible = True
        txtNumPedido.Visible = True
...
Private Sub txtCorretor_LostFocus()
    ...
    '1. Se código do corretor já preenchido, nada a fazer.
    If gM46V999.gfPreenchido(lblCodCorretor.Caption) Then
        Exit Sub
    End If
    ...
    '3. Se não preenchido, encerra processo.
    ...
    '   Obtém registro e posiciona dados.
    ...
    '6. Se preenchido com menos de 3 posições, encerra processo.
    ...
    '7. Obtém registros com informação do usuário.
    ...
    If gM46V999.gfObtCorretor(2, lCorretor, lSaida, lMensagem) = False Then
        ...
        Exit Sub
    End If
    'c) Se campo de saída estiver vazio, nada a fazer.
    ...
    With frmT46V004A.flxConsulta
        ...
        For lI = 1 To lQuaCorretores
            .TextMatrix(lI, 0) = lSaida(lI, 1)  'Código do corretor.
...
Private Sub txtProdutor_LostFocus()
    ...
    '1. Se código do produtor já preenchido, nada a fazer.
    If gM46V999.gfPreenchido(lblCodProdutor.Caption) Then
        Exit Sub
    End If

    '2. Se produtor não preenchido, encerra processo.
    ...
    '5. Se produtor é numérico e tamanho superior a 4, emite mensagem e encerra processo.
    If IsNumeric(lProdutor) Then
        If Len(lProdutor) > 4 Then
            txtProdutor.Text = ""
            Call gM46V999.gpGraLog(1, "FGE0084 - Produtor inválido (código do produtor com mais de 4 posições).")
            Screen.MousePointer = vbDefault
            Exit Sub
        End If
    End If
    ...
    '7. Obtém registros de produtores.
    ...
    End If
    If IsNumeric(lProdutor) Then
        'Se produtor numérico, trata-se do código do produtor.
        ...
        If rsPRODUTOR.EOF = True Then
            txtProdutor.Text = ""
            Call gM46V999.gpFecha2(bdP0044700)
            Call gM46V999.gpGraLog(1, "FGE0085 - Produtor inválido (código de produtor não localizado: " & _
                                      lProdutor & ").")
            lstAviso.Visible = False
            lstAviso.Refresh
            Screen.MousePointer = vbDefault
        ...
        End If
    Else
        'Se produtor não numérico, trata-se do nome do produtor.
        ...
        If rsPRODUTOR.EOF = True Then
            ...
            Call gM46V999.gpGraLog(1, "FGE0086 - " & gM46V999.gPrefixo & _
                                      "Produtor inválido (não localizado produtor com nome " & _
                                      Trim$(lProdutor) & ").")
            lstAviso.Visible = False
            lstAviso.Refresh
            Screen.MousePointer = vbDefault
    ...
    With frmT46V004A.flxConsulta
        ...
        While Not rsPRODUTOR.EOF
            lI = lI + 1
            .TextMatrix(lI, 0) = rsPRODUTOR!Cod_Prod    'Código do produtor.
...
'Procedure utilizada para selecinar ou desmarar arquivos para exclução

'Parâmetro de entrada...pFlag...true = selecionar todos e false = desmarcar todos.
