...
Begin VB.Form frmT46V174A 
   ...
   Caption         =   "Lista de cotações sem agendamento de inspeção"
   ClientHeight    =   9015
   ClientLeft      =   120
   ClientTop       =   2190
   ClientWidth     =   12390
...
Public gNosNumeroInsp As String     'Nosso número.
Public gNumItemInsp As String   'Número do item da apólice.
Public gNumPedidoInsp As String     'Número de pedido.

Public gAbortSolic As Boolean    'Flag para abortar solicitação de inspeção da tela T46V175B
...
Public Sub cmdSolicitar_Click()
    ...
    'Essa rotina foi re-estruturada para contemplar a requisição da ficha acima.

    On Error GoTo ControleErro

    With flxCotacoes
        If .Rows = 1 Then Exit Sub
        gNumPedidoInsp = .TextMatrix(.RowSel, 1)    'Número do pedido.
        gNumItemInsp = .TextMatrix(.RowSel, 2)   'Número do item.
        gNosNumeroInsp = .TextMatrix(.RowSel, 7)    'Nosso número.

        If MsgBox("Deseja Solicitar Inspeção para o Pedido " & gNumPedidoInsp & " Item " & gNumItemInsp & " ?", vbQuestion + vbYesNo, "Atenção") = vbNo Then
            Exit Sub
        End If
        ...
        If lrst.RecordCount > 1 Then
            If MsgBox("Existe mais de um item para o Pedido " & gNumPedidoInsp & vbCrLf & "Deseja gerar inspeção para todos os itens com os mesmos dados de inspeção?", vbQuestion + vbYesNo, "Atenção") = vbNo Then
        ...
        If objPedido.Colecao.Count = 0 Then
            Call gM46V999.gpGraLog(1, "INSP0174 - Não há cotação dentro do prazo de solicitação de inspeção.", vbInformation)
            Exit Sub
        End If
    ...
    MsgBox "Gerada " & IIf(lrst.RecordCount > 1, "solicitações", "solicitação") & " de inspeção para " & IIf(lrst.RecordCount > 1, lrst.RecordCount & " itens", "o item " & gNumItemInsp) & " da cotação " & gNumPedidoInsp & "." & vbLf & "Utilize a opção de Conexão Internet Yasuda para completar o processo de solicitação.", vbInformation
...
Private Sub flxCotacoes_DblClick()
    ...
    If flxCotacoes.Rows = 1 Then
        Call gM46V999.gpGraLog(1, "INSP0174 - Não há cotação Syas dentro do prazo de solicitação de inspeção.", vbInformation)
        TabInspecao.Tab = 1
        cmdSolicitar.Enabled = False
        Exit Sub
    End If
...
Private Sub Form_Load()
    ...
    With flxCotacoes
        ...
        .TextMatrix(0, 8) = "Insp.Obrigatória?"
            ...
            For Each lobjstcPedLoc In objstcPedido.ColecaoPedLoc
                If lobjstcPedLoc.Insp_Obr = True And lobjstcPedLoc.Insp_Agend = False Then
                    ...
                    If lobjstcPedLoc.Cod_Ramo = e_CodRamo_Condominio Then
                        .TextMatrix(lCount, 0) = "Condomínio"
                    ...
                    Else
                        .TextMatrix(lCount, 0) = "Não definido"
                    End If
                    ...
                    Select Case Len(CStr(objstcPedido.Num_Cgc_Cpf))
                    Case Is > 11
...
Function mfPesqCotAgend()
    Dim bdBasDados As ADODB.Connection    'Conexão do Banco local das TAB_PED temporárias
    ...
    If gM46V999.gTipUsuario = e_Tipo_Usu_Corretor Or gM46V999.gTipUsuario = e_Tipo_Usu_CorretorYasuda Then
        ...
        'Se não encontrou registro, informa usuário.
        If lrst.EOF Then
            lMensagem = "FNL0174 - Não há cotações pendentes de solicitação de inspeção." & vbLf
            Call gM46V999.gpGraLog(1, lMensagem)
        End If
        ...
        Do While Not lrst.EOF
            ...
            If lrst("Cod_Ramo") = e_CodRamo_Condominio Then
                flxCotacoes.TextMatrix(lCount, 0) = "Condomínio"
            ...
            Else
                flxCotacoes.TextMatrix(lCount, 0) = "Não definido"
            End If
