...
Function mfGeraCopia(pAntNosNumero As String, pNumPI As Long, pDatRecCia As Long, ByRef pNovNosNumero As String, ByRef pMensagem As String) As Boolean
    ...
    Dim lNovNumPedido As String     'Novo: número do pedido.
    ...
    pMensagem = "AGE0009 - " & gM46V999.gPrefixo & "Confirma cópia deste documento (Rotina de cópia de cotação)?"
    ...
    Call gM46V999.gpGraLog(0, "AGE0002 - Processo concluído: cópia realizada para pedido " & lNovNumPedido & _
                            " (nosso número " & pNovNosNumero & ").")
...
Private Sub mpConsisteComum(pobjstcPed As stcA46V702B, pobjstcPedLoc As stcA46V708B, pCod_Retorno As Integer)
    ...
    ElseIf gM46V999.gfPreenchido(lretorno) = True Then    'retornou sem erros
        Call mpInformarErro("Consistência do comum - " & lretorno, 2, True)
    End If
...
Private Sub mpInformarErro(ByVal pTexto As String, pStatus As Byte, pDeslocado As Boolean)
    Dim lPos As Long  'Posição de caracter.
...
Retorno:
    ...
    If gM46V999.gfPreenchido(txtNumeroPI) Then
        ...
        If lobjCtrlEmis.Colecao.Count <> 0 Then
            ...
            If lobjstcCtrlEmis.Dat_Rec_Cia < DateAdd("d", -15, Date) And gM46V999.PerfilAcesso <> e_Perfil_DIG_Gestor And lobjstcCtrlEmis.Num_Apol = 0 Then
                Call gM46V999.gpGraLog(1, "FGE0079 - Data de recebimento inválida (inferior a 15 dias da data de " & Date & ")")
    ...
    If CDate(frmT46V012A.txtDataRecebimento) < DateAdd("d", -15, Date) And gM46V999.PerfilAcesso <> e_Perfil_DIG_Gestor Then
        Call gM46V999.gpGraLog(1, "FGE0080 - Data de recebimento inválida (inferior a 15 dias da data de " & Date & ")")
    ...
    If gM46V999.gfPreenchido(txtPropostaSyas.Text) Then
        If Not gM46V999.gBaseLocal Then
            ...
            If lretorno = 1 Then
                ...
                Call gM46V999.gpGraLog(1, "FNL0041 - Número da proposta não localizado (" & txtPropostaSyas.Text & ").")
            ElseIf lretorno = 2 Then
                '****Transmissão localizada com DV diferente******
            ...
            Else
                ...
                If gM46V999.gfCalNosNumero(lNovNosNumero, lMensagem) = False Then
                    Call gM46V999.gpGraLog(1, "FNP0021 - Pesquisa Digitação: " & lMensagem)
                    Exit Sub
                End If

                If gM46V999.gfCalNumPedido(lNovNumPedido, lMensagem) = False Then
                    Call gM46V999.gpGraLog(1, "FNP0022 - Pesquisa Digitação: " & lMensagem)
                    Exit Sub
                End If
...
RegistroLocalizado:
                ...
                If lObjPed.Colecao.Count <> 0 Then
                    ...
                    'Ficha 901824
                    'Vai verificar se existem apontamentos na triagem para RELACAO DE BENS, SOMENTE PARA PREDIO e SOMENTE PARA CONTEUDO, incluindo os textos para a emissão do Booklet
                    'Vai verificar se o campo PRIMEIRA PARCELA ou SEGUNDA PARCELA estão preenchidos, se sim, deverá abrir a tela de vencimentos do Syas
...
'********************Variáveis Declaradas para o processamento************
    ...
    Dim lbdRamosDiversos As ADODB.Connection    'Conexão do Banco local das FTP_TAB_PED
    ...
    gM46V999.gQuaErros = 0
    lQtdErro = mQtdErros
    '1. Consitência dos dados de comum
    'a) Itens.
    ...
    If mobjstcPed.ColecaoPedLoc.Count = 0 Then
        Call mpInformarErro("Não localizado registro de item", 2, True)
    End If
    ...
    If mobjstcPed.ColecaoPedLoc.Count > 1 Then
        If mobjstcPed.Num_Apol_An = 0 Then
            'Verifico se existe algum item que é renovação
    ...
    'Busca os valores dos prêmios de transmissão
    If Not gM46V999.gfObtRegistro(lbdRamosDiversos, lSelect, lrst, lMensagem) Then
        Call gM46V999.gpMenErro(lMensagem)
        Exit Sub
    End If
    ...
    If Not lrst.EOF Then
        If Format(mobjstcPed.VAL_PR, "standard") <> Format(lrst("VAL_PR"), "standard") Then
            Call mpInformarErro("Valor do prêmio divergente - Transmitido = R$" & Format(lrst("VAL_PR"), "standard") & ", calculado = R$" & Format(mobjstcPed.VAL_PR, "standard"), 2, True)
        End If

        If Format(mobjstcPed.Val_Pr_Ref_Tarif, "standard") <> Format(lrst("VAL_PR_REF_TARIF"), "standard") Then
            Call mpInformarErro("Valor do prêmio tarifário divergente - Transmitido = R$" & Format(lrst("VAL_PR_REF_TARIF")) & ", calculado = R$" & Format(mobjstcPed.Val_Pr_Ref_Tarif, "standard"), 2, True)
        End If
    End If
    ...
    If mobjstcPed.ColecaoPedJuros.Count = 0 Then
        Call mpInformarErro("Juros não localizado." & lMensagem, 2, True)
    End If
            ...
            If Format(mobjstcPed.Val_PriParcela, "standard") <> Format(gM46V999.gfTratarNulo(lrst("val_pr_antec"), tpoNumerico), "standard") Then
                ...
                Else
                    If gM46V999.gfTratarNulo(lrst("val_pr_antec"), tpoNumerico) <> 1 Then
                        Call mpInformarErro("Valor da 1ª parcela divergente - Transmitido = R$" & Format(gM46V999.gfTratarNulo(lrst("val_pr_antec"), tpoNumerico), "standard") & ", calculado = R$" & Format(mobjstcPed.Val_PriParcela, "standard"), 2, True)
                    Else
            ...
            If cboOpcao.ListIndex <> 4 Then
                ...
                If gM46V999.gTipUsuario = e_Tipo_Usu_Yasuda And gM46V999.gTipoAcesso = DIGITACAO Then
                    ...
                    If frmT46V190A.TodosItensCorretos = False Then
                        Call gM46V999.gpGraLog(1, "Todos os itens que tem inspeção obrigatória devem possuir dados de contato.")
                        Screen.MousePointer = vbDefault
                        Exit Sub
                    End If
...
Private Sub mpValidaParcela(pobjstcPed As stcA46V702B, pobjstcPedLoc As stcA46V708B)
    ...
    Dim lMaxParcelaAux As Integer     'Número máximo de parcelas permitido
    ...
    If pobjstcPed.Cod_Tip_Doc = e_TipDoc_Proposta Then
        ...
        'ENDOSSO
        'Após o cálculo é que será consistido se a postecipada é permitida ou não.
        'através da variável lMaxParcelaAux.
    ...
    If pobjstcPed.Cod_Tip_Doc = e_TipDoc_Cotacao Then
        ...
        'Se = Cotação então atualiza o campo
        If pobjstcPed.Qtd_Parc > lMaxParcela Then
            pobjstcPed.Qtd_Parc = lMaxParcela
        End If
    Else
        'Verifica se o numero informado é maior que o permitido
        If lQutdeParcelas > lMaxParcela Then
            If pobjstcPed.ColecaoPedVenc.Count > 0 And gM46V999.gTipUsuario = e_Tipo_Usu_Yasuda Then
                For Each lobjstcParc In pobjstcPed.ColecaoPedVenc
                    If lobjstcParc.DAT_VENC > pobjstcPed.Dat_Ter_Vig Then
                        Call mpInformarErro("Quantidade de parcelas inválida (Data da última parcela não deve ultrapassar " & _
                                            gM46V999.gfForData(2, lobjstcParc.DAT_VENC) & ". " & val(gM46V999.gfDifDatas(lDataUltVencimento, pobjstcPed.Dat_Ter_Vig)) & _
                                          " dias antes do témino da vigência  )." & vbCrLf & "Número máximo de parcelas permitido: " & lMaxParcela, 2, True)
                        Exit Sub
                    End If
            ...
            Else
                If lDataUltVencimento = 0 Then
                    Call mpInformarErro("Quantidade de parcelas inválida (Data da última parcela deve ser inferior a data de término da vigência)." & vbCrLf & "Número máximo de parcelas permitido: " & lMaxParcela, 2, True)
                Else
...
'Procedure: consiste dados comuns
    ...
    Else
        ...
        'a definição se "O" ou "E" é feita dentro da gM46V999.gfVerEtiqueta
    ...
    If Right(pobjstcPed.Dat_Ini_Vig, 4) = "0229" Then
        Call mpInformarErro("Vigência inválida (data de início com ano " & _
    ...
    If pobjstcPed.Cod_Tip_Doc = e_TipDoc_Proposta Then
        ...
        With clsA46V102
            ...
            'Abrindo form para validação da agência e conta.
            If pobjstcPed.Num_Cta_Bas_Cor <> 0 Then
                ...
                frmT46V110A.txtDebAgencia.Text = pobjstcPed.gAgenciaDig    '1001180 - Validação de DV na Triagem
            ...
            If Err <> 0 Then
                Call mpInformarErro("FTE0009 - " & gM46V999.gPrefixo & "Erro ao consistir dados comuns nível 2 (" & _
                                    Err & " - " & Error & ").", 2, True)
                On Error GoTo 0
                Exit Sub
            End If
        ...
        Case e_TipEmissao_RenYasuda
            ...
            If Mid(mDatTerVig + 30, 7, 4) & Mid(mDatTerVig + 30, 4, 2) & Mid(mDatTerVig + 30, 1, 2) < pobjstcPed.Dat_Ini_Vig Then
                Call mpInformarErro("Início de vigência superior a 30 dias " & _
                                    "do final de vigência da apólice anterior! Favor calcular seguro novo.", 2, True)
            End If
        End Select
        ...
        If pobjstcPed.objstcCtrlEmis.Num_Bloq & pobjstcPed.objstcCtrlEmis.DIG_BLOQUETO <> 0 Then
            ...
            If Right(lAuxiliar, 1) <> lDigito Then
                Call mpInformarErro("Pagamento antecipado com bloqueto - número do bloqueto com dígito inválido.", 2, True)
            End If
        End If
        ...
        '        Dim i As Integer
        '
        '        If MsgBox("Deseja informar beneficiário(s) para o(s) item(ns)?", vbQuestion + vbYesNo) = vbYes Then
            ...
            Else
                ...
                For lI = 1 To (pobjstcPed.Qtd_Parc)
                    If IsDate(frmT46V121A.txtDtVenc(lI).Text) = False Then
                        lMensagem = "FGE0082 - Data de vencimento da parcela " & lI + 1 & " inválida (não preenchida)."
                        Call mpInformarErro(lMensagem, 2, True)
                    Else
                        Set lObjstcVenc = New stcA46V713B
                        Set lObjstcVenc.M46V999 = gM46V999
            ...
            If (UCase(pobjstcPedLoc.objstcPedObs.Observacao) Like "*VENC*" And Not UCase(pobjstcPedLoc.objstcPedObs.Observacao) Like "*VENCIDO*") Or _
               ...
               UCase(pobjstcPedLoc.objstcPedObs.Observacao) Like "*PAGTO*" Then
                ...
                If gM46V999.gfPreenchido(frmT46V121A.txtDtVenc(1).Text) = True Then
                    For lI = 1 To (pobjstcPed.Qtd_Parc)
                        If IsDate(frmT46V121A.txtDtVenc(lI).Text) = False Then
                            lMensagem = "FGE0082 - Data de vencimento da parcela " & lI & " inválida (não preenchida)."
                            Call mpInformarErro(lMensagem, 2, True)
                        ElseIf gM46V999.gfForData(1, frmT46V121A.txtDtVenc(lI).Text) > pobjstcPed.Dat_Ter_Vig Then
                            lMensagem = "FGE0082 - Data de vencimento da parcela " & lI & " inválida (maior que o término da vigência)."
                            Call mpInformarErro(lMensagem, 2, True)
                        ElseIf gM46V999.gfForData(1, frmT46V121A.txtDtVenc(lI).Text) < gM46V999.gHoje Then
                            lMensagem = "FGE0082 - Data de vencimento da parcela " & lI & " inválida (menor que a data atual)."
                            Call mpInformarErro(lMensagem, 2, True)
                        Else
                            Set lObjstcVenc = New stcA46V713B
                            Set lObjstcVenc.M46V999 = gM46V999
    ...
    Else
        Do While lrsTabela.EOF = False
            ...
            If gM46V999.Check(lrsTabela("Cod_Unicidade")) <> pDigVer Then
                pRetorno = 2
            Else
                'Encontrado transmissão com mesmo DV
                pRetorno = 3
                Exit Do
            End If
            lrsTabela.MoveNext
...
Private Function mfBuscarDadosSiscad(pNum_Apol As Double, _
    ...
    ''**************************OBSERVAÇÃO********************
    ''******Endereço do segurado estão no tab_ped_ender*******
    ''******Endereço de cobrança está na tab_ped**************
    ''********************************************************
    ...
    lSelect = lSelect & " num_cgc_cpf = " & lCPF_CNPJ
    ...
    If Not rsRamosDiversos.EOF Then
        ...
        If pNum_Apol <> 0 Then
            'Pesquisar na tabela tab_ctrl_emis em qual PI a apólice q está sendo renovada foi emitida...
        ...
        If Not rsSyasCob.EOF Then
            ...
            If Format(mobjstcPed.Num_CEP, "00000000") <> Format(rsSyasCob("Num_CEP"), "00000000") Then
                Call mpInformarErro("CEP de cobrança divergente - " & _
            ...
            If Trim(mobjstcPed.Nom_Logr) <> Trim(rsSyasCob("NOM_LOGR")) Then
                ...
                Else
                    Call mpInformarErro("Logradouro de cobrança divergente - " & _
                ...
                If lCepCobDif = False Then
                    If mobjstcPed.Dsc_Compl_Logr Like "*" & Trim(rsSyasCob("DSC_COMPL_LOGR")) & "*" Or _
                       Trim(rsSyasCob("DSC_COMPL_LOGR")) Like "*" & mobjstcPed.Dsc_Compl_Logr & "*" Then
                        ''Altero o siscad com a informação da transmissão.
                    Else
                        Call mpInformarErro("Complemento de cobrança divergente - " & _
            ...
            If Trim(mobjstcPed.Nom_Bairro) <> Trim(rsSyasCob("NOM_BAIRRO")) Then
                ...
                Else
                    Call mpInformarErro("Bairro de cobrança divergente - " & _
    ...
    lSelect = "Select * from ged.dbo.tab_ctrl_emis Where num_cgc_cpf = " & mobjstcPed.Num_Cgc_Cpf & " and Num_PI <> " & val(txtNumeroPI.Text) & " order by dat_ini_vig desc"
    ...
    If lMsgNomDiferente <> "" Then
        Screen.MousePointer = vbDefault
        MsgBox "Comparação de titularidades de proponente/segurado divergente. Favor confirmar titularidade no site da Receita Federal." & vbCrLf & lMsgNomDiferente
        Screen.MousePointer = vbHourglass
...
Private Function mfConsistirDados() As Boolean
    ...
    Dim lDigitoNP As Integer    'Dígito.
    ...
    '1. Consistências para a pesquisa
    '1a. Efetuando consistência no Número do PI

    If Not gM46V999.gfPreenchido(txtNumeroPI.Text) Then
        Call gM46V999.gpGraLog(1, "FNP0020 - Número do PI não preenchido.")
    ...
    If Not gM46V999.gfPreenchido(txtDigito.Text) Then
        Call gM46V999.gpGraLog(1, "FNP0041 - Dígito do PI não preenchido.")
    ...
    If UCase(txtDigito.Text) <> lDigito Then
        ...
        If UCase(gM46V999.gNomUsuario6) = "TJGOME" Or UCase(gM46V999.gNomUsuario6) = "LVDELA" Or UCase(gM46V999.gNomUsuario6) = "SIMONY" Or UCase(gM46V999.gNomUsuario6) = "MFBARR" Then
            Call MsgBox(lDigito)
        Else
            Call gM46V999.gpGraLog(1, "FNP0041 - Dígito do PI inválido.")
        End If
    ...
    '1c. Efetuando consistência na data de recebimento

    If Not gM46V999.gfPreenchido(txtDataRecebimento.Text) Then
        Call gM46V999.gpGraLog(1, "FGE0079 - Data de recebimento inválida(não preenchida).")
    ...
    ElseIf Not IsDate(txtDataRecebimento.Text) Then
        Call gM46V999.gpGraLog(1, "FGE0079 - Data de recebimento inválida(" & txtDataRecebimento.Text & ").")
    ...
    ElseIf CVDate(txtDataRecebimento.Text) > Date And gM46V999.PerfilAcesso <> e_Perfil_DIG_Gestor Then
        Call gM46V999.gpGraLog(1, "FGE0079 - Data de recebimento inválida (superior a data de hoje:" & Date & ")")
    ...
    '1d. Efetuando consistência no Número da proposta
    If lOpcaoSelecionada = 1 Then
        If Not gM46V999.gfPreenchido(txtPropostaSyas.Text) Then
            Call gM46V999.gpGraLog(1, "FNP0022 - Número da Proposta não preenchido.")
        ...
        ElseIf gM46V999.CDblx(txtPropostaSyas.Text) = 0 Then
            ...
            Exit Function
        End If                                                            'informado o número da proposta.
        ...
        If Right(txtPropostaSyas.Text, 1) <> lDigitoNP Then
            Call gM46V999.gpGraLog(1, "FNL0041 - Dígito do número da proposta está inválido.")
            If txtPropostaSyas.Visible = True And txtPropostaSyas.Enabled = True Then
                txtPropostaSyas.SetFocus
            End If
            Exit Function
        ...
        '1f. Efetuando consistência no Dígito Verificador
        If Not gM46V999.gfPreenchido(txtDV.Text) Then
            Call gM46V999.gpGraLog(1, "FNP0023 - Dígito Verificador não preenchido.")
    ...
    If lOpcaoSelecionada = 3 Then
        ...
        Else
            If Not IsNumeric(txtMTVVD.Text) Then
                Call gM46V999.gpGraLog(1, "FNP0041 - Proposta VVD inválida.")
                txtMTVVD.SetFocus
                Exit Function
            End If
    ...
    If lOpcaoSelecionada = 4 Then
        ...
        Else
            If Not IsNumeric(txtMTVVD.Text) Then
                Call gM46V999.gpGraLog(1, "FNP0041 - a Referencia Marsh inválida.")
                txtMTVVD.SetFocus
                Exit Function
            End If
            If Len(txtMTVVD.Text) < 6 Then
                Call gM46V999.gpGraLog(1, "FNP0041 - Referencia Marsh inválida.")
                txtMTVVD.SetFocus
                Exit Function
            End If
        End If
...
Private Sub mpLimparErros()
    ...
    With frmT46V012B.flxResultado
        ...
        .TextMatrix(1, 1) = "Consistências"
        .TextMatrix(2, 1) = "Prêmio"
        For lI = 0 To 1
            .Row = lI
            .Col = 0
...
Private Function mfIncluirSiscad(pMensagem As String) As Boolean
    ...
    lInsert = lInsert & "0,"
    'Ficha 901659 - não utilizar o gM46V999.gNomUsuario pois está dando erro por ser maior que o campo da tabela
...
Private Function mfAlterarSiscad(pMensagem As String) As Boolean
    ...
    'Ver como o nome será atualizado
    'Ficha 900417
...
'Carregar os endereço de cobrança
    Dim lInsert As String
    Dim bdRamosDiversos As ADODB.Connection
...
Private Sub mnuExiRelatorio_Click()
    Dim lAppExcel As excel.Application    'Aplicação Excel.
    ...
    lData = InputBox("Data de pesquisa de triagem", "Relatório de triagem", Date)

    If IsDate(lData) = False Then
        Call gM46V999.gpGraLog(1, "Data inválida!")
        Exit Sub
    End If
    ...
    lwksPrincipal.Cells(lLinha, 5) = "Usuário"
...
Function mfVerDizer(ByRef pobjstcPed As stcA46V702B, ByRef pMensagem As String) As Boolean
    ...
    If gM46V999.gTipUsuario = e_Tipo_Usu_Yasuda Then
        If lrsTabela("ind_relbens") = 2 Then
            'Procurar se há dizer na TAB_PED_ESPEC com o código 8401 e exclui
            codDizer = 8401
        ...
        If lrsTabela("ind_relbens") = 1 Then
            'Procurar dizer na TAB_PED_DIZER_PAD com o código 8401
            codDizer = 8401
        ...
        If lrsTabela("ind_segpredio") = 2 Then
            'Procurar se há dizer na TAB_PED_ESPEC com o código 8565 e exclui
            codDizer = 8565
            If Not mfDeletaDizer(mobjstcPed, codDizer, pMensagem) Then
                Exit Function
            End If
        ...
        If lrsTabela("ind_segpredio") = 1 Then
            'Procurar dizer na TAB_PED_DIZER_PAD com o código 8565
            codDizer = 8565
            If Not mfProcuraDizer(mobjstcPed, codDizer, pMensagem) Then
                Exit Function
            End If
        ...
        If lrsTabela("ind_segconteudo") = 2 Then
            'Procurar se há dizer na TAB_PED_ESPEC com o código 8621 e exclui
            codDizer = 8621
            If Not mfDeletaDizer(mobjstcPed, codDizer, pMensagem) Then
                Exit Function
            End If
        ...
        If lrsTabela("ind_segconteudo") = 1 Then
            'Procurar dizer na TAB_PED_DIZER_PAD com o código 8621
            codDizer = 8621
            If Not mfProcuraDizer(mobjstcPed, codDizer, pMensagem) Then
                Exit Function
            End If
        ...
        If pobjstcPed.objstcCtrlEmis.Cod_Iden_Pag_Antec = 1 Then
            If pobjstcPed.objstcCtrlEmis.Num_Bloq <> 0 Then    'carnê
                ForPagamento = 1    'carnê
                TipoPagamento = 1    'antecipado
            Else
                ForPagamento = 2    'débito
                TipoPagamento = 1    'antecipado
            End If
        Else
            '######### A 30 dias #########
            ...
            For lI = 0 To (pobjstcPed.Qtd_Parc - 1)
                If lI < 11 Then
                    If IsDate(frmT46V121A.txtDtVenc(lI + 1).Text) = False Then
                        pMensagem = "FGE0082 - Data de vencimento da parcela " & lI + 1 & " inválida (não preenchida)."
...
Function mfProcuraDizer(ByRef pobjstcPed As stcA46V702B, lCodDizer As Integer, ByRef pMensagem As String) As Boolean
    ...
    If lrsTab_Ped_Dizer_Pad.EOF Then
        MsgBox "Mensagem Padrão " & lCodDizer & " não encontrada!" & vbCrLf & "Favor efetuar cadastro do Dizer para prosseguir emissão!", vbInformation
        Exit Function
    Else
...
mfProcuraDizer_Err:
    MsgBox "Erro na função mfProcuraDizer - Contactar Sistemas"
End Function
...
mfDeletaDizer_Err:
    MsgBox "Erro na função mfDeletaDizer - Contactar Sistemas"
End Function
...
Private Function ReentradaImobiliario(ByVal pNum_pi As Long, ByVal pNosso_Numero As String, ByRef pNosso_Numero_Novo As String, ByVal pMensagem As String) As Boolean
    ...
    If lrs.EOF Then
        pMensagem = "Este PI não existe ou não é Imobiliario!"
        GoTo SAIDA
    End If
    
    'Limpa Transmissões anteriores
...
'    End If
'
'    ' Tentar encontrar a Inspeção a partir do PI e Triagem
...
'    End If
'        Else
'            ' Não encontrou, tentar então obter o nosso número a partir da TAB_PED
