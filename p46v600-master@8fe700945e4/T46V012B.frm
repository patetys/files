...
Begin VB.Form frmT46V012B 
   ...
   Begin Threed.SSPanel panAviso 
      ...
      Begin VB.Label lblboleto 
         ...
         Caption         =   "Bloqueto da transmissão:"
         ForeColor       =   &H00C00000&
         Height          =   285
         Left            =   3720
         TabIndex        =   10
...
Private Sub cmdEmitir_Click()
    ...
    If txtPagAntBloqueto.Visible = True Then
        ...
        ' Validação removida devido ao projeto de boleto registrado
'        If Right(lAuxiliar, 1) <> lDigito Then
'            Call gM46V999.gpGraLog(1, "FGE0059 - Pagamento antecipado com bloqueto - número do bloqueto com dígito inválido.")
'            Exit Sub
'        End If
...
Retorno:
    If gM46V999.gEfetuouCalculo = True Then
        ...
        ' se tiver então eu não permito emitir, somente suspender
    ...
    If lTipoTransmissao > 0 Then
        ...
        If lOK <> "" Then
            MsgBox "Pi: " & mobjstcPed.objstcCtrlEmis.NUM_PI & " será " & lOK & ".", vbInformation
...
Private Sub panSiscad_Click()
    ...
    Call Shell(gM46V999.gEndExeSiscad & " " & Format(mobjstcPed.Num_Cgc_Cpf, String(14, "0")) & "0" & _
...
Public Sub mpAtualizaDadosSegurado()
    ...
    lSelect = lSelect & " num_cgc_cpf = " & mobjstcPed.Num_Cgc_Cpf
    ...
    If Not rsDiversos.EOF Then
        ...
        lSelect = lSelect & " and num_cgc_cpf = " & mobjstcPed.Num_Cgc_Cpf
