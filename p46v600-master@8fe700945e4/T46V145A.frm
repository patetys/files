...
Begin VB.Form frmT46V145A 
   ...
   Begin VB.Label lblAviso 
      Caption         =   "*Para endosso de inclusão, informe o número 9999."
      ForeColor       =   &H000000C0&
      Height          =   375
      Left            =   4425
      TabIndex        =   10
...
Option Explicit
Dim mNumeroApolice As Double                    'Número da Apólice
Dim mNumerosItens As String                 'Números dos itens desta apólice pertencentes a esta cotação
...
Public Sub BuscaItemEndosso(txtRenApolice As String, txtRenItem As String)
    ...
    If Not gM46V999.gfPreenchido(txtRenApolice) Then
        Call gM46V999.gpGraLog(1, "FGE0069 - " & gM46V999.gPrefixo & _
                                  "Número da apólice inválido (não preenchido).")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    If Not IsNumeric(txtRenApolice) Then
        Call gM46V999.gpGraLog(1, "FGE0069 - " & gM46V999.gPrefixo & _
                                  "Número da apólice inválido (não numérico).")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    If Val(txtRenApolice) = 0 Then
        Call gM46V999.gpGraLog(1, "FGE0069 - " & gM46V999.gPrefixo & _
                                  "Número da apólice inválido (zerado).")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    ...
    If Not gM46V999.gfPreenchido(txtRenItem) Then
        Call gM46V999.gpGraLog(1, "FGE0070 - " & gM46V999.gPrefixo & _
                                  "Número do item da apólice inválido ( não preenchido).")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    If Not IsNumeric(txtRenItem) Then
        Call gM46V999.gpGraLog(1, "FGE0070 - " & gM46V999.gPrefixo & _
                                  "Número do item da apólice inválido (não numérico).")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    If Val(txtRenItem) = 0 Then
        Call gM46V999.gpGraLog(1, "FGE0070 - " & gM46V999.gPrefixo & _
                                  "Número do item da apólice inválido (zerado).")
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
    ...
    If rsAPOL.EOF = True Then
        ...
        End If
        If rsAPOL.EOF = True Then
            lMensagem = "FNL0008 - Apólice " & txtRenApolice & " não localizada."
        Else
            lMensagem = "FNL0009 - Item  " & txtRenItem & " não localizado na apólice " & _
                        txtRenApolice & "."
        End If
...
Private Sub Form_Activate()
    If flxConsulta.Rows = 1 Then
        MsgBox "Não existem outros item para endosssar", vbExclamation
        Unload Me
        Exit Sub
    End If
End Sub
...
Private Sub Form_Load()
    ...
    'Preciso identificar os itens pertencentes a esta cotação para não incluí-las na pesquina
    mNumerosItens = ""
    'Monto uma string com os números dos itens
...
Private Sub mpCarregarGrid(pItem As Integer)
    ...
    'Consulto todos os itens desta apólice que não estão nesta cotação
    ...
    With flxConsulta
        While Not rsP00APOL.EOF
            If InStr(mNumerosItens, "'" & Format(rsP00APOL("Num_Item"), "0000") & "'") = 0 Then
                ...
                .TextMatrix(.Rows - 1, 0) = rsP00APOL("Num_Item")   'Número do item.
