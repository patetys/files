...
'Verificar as informações digitadas.
    For lI = 1 To flxBens.Rows - 1
        ...
        If flxBens.TextMatrix(lI, 2) = "" And gM46V999.CDblx(flxBens.TextMatrix(lI, 3)) = 0 Then
            If flxBens.Rows = 2 Then
                gCarga = True
                frmT46V101A.optBens(1).Value = True
                gCarga = False
                Exit For
            Else
                MsgBox "FNL0053 - Preencher corretamente a linha " & lI & " da tela de Relação de Bens.", vbInformation
        ...
        Else
            gCarga = True
            frmT46V101A.optBens(0).Value = True
            gCarga = False
            Exit For
        End If
    Next lI
    ...
    For lI = 1 To flxBens.Rows - 1
        If flxBens.TextMatrix(lI, 2) <> "" Then
        flxBens.TextMatrix(lI, 2) = gM46V999.gfRetirarCaracterEspecial(flxBens.TextMatrix(lI, 2), False, True, True)
            CmbBens.ListIndex = gM46V999.gfRetornarListIndex(CmbBens, "999")
            flxBens.TextMatrix(lI, 1) = CmbBens.Text
            flxBens.TextMatrix(lI, 4) = 999
        End If
    Next lI
...
Private Sub flxBens_Click()
    ...
    lTexto = Text1.Text
    If Trim(lTexto) <> "" Then
        flxBens.TextMatrix(mLinhaFocus, mColunaFocus) = gM46V999.gfRetirarCaracterEspecial(lTexto, False)
    End If
...
Private Sub flxBens_DblClick()
    mLinhaFocus = flxBens.Row
    mColunaFocus = flxBens.Col
    ...
    With Text1
        If .Visible Then
            .Text = flxBens.TextMatrix(mLinhaFocus, mColunaFocus)
            .SelStart = Len(.Text) + 1
        End If
    End With
End Sub
...
'        End If
        ...
        .TextMatrix(0, 2) = "Descrição (Marca/Tipo/Mod/Núm Série)" '"Descrição (Marca, Mod. Núm Série)"  '1200539 - Tarifa 1038 e 1039 Junho 2012 - Item 9
        .TextMatrix(0, 3) = "Valor do bem (R$)"
        .TextMatrix(0, 4) = ""
        .ColWidth(0) = 660
        .ColWidth(2) = 4940
        .ColWidth(3) = 0 '1515
...
Private Sub ExibirCelula()
    Dim lOK As Boolean
    If flxBens.Col <= flxBens.FixedCols - 1 Or flxBens.Row <= flxBens.FixedRows - 1 Then
        Exit Sub
    End If
    ...
    Case Else
        '            Text1.Move flxBens.CellLeft - Screen.TwipsPerPixelX, flxBens.CellTop + 550 - Screen.TwipsPerPixelY, flxBens.CellWidth + Screen.TwipsPerPixelX * 2, flxBens.CellHeight + Screen.TwipsPerPixelY * 2
        Text1.Move flxBens.CellLeft - Screen.TwipsPerPixelX, flxBens.CellTop - Screen.TwipsPerPixelY, flxBens.CellWidth + Screen.TwipsPerPixelX, flxBens.CellHeight + Screen.TwipsPerPixelY
        Text1.Text = flxBens.Text

        If Len(flxBens.Text) = 0 Then
            If mLinhaFocus > 1 Then
                Text1.Text = flxBens.TextMatrix(mLinhaFocus - 1, mColunaFocus)
            End If
        End If
...
Private Sub Text1_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        KeyAscii = 0
        lTexto = Text1.Text
        flxBens.TextMatrix(mLinhaFocus, mColunaFocus) = lTexto

        If flxBens.Col < flxBens.Cols - 1 Then
            flxBens.Col = flxBens.Col + 1
        Else
            flxBens.Col = 1
            If flxBens.Row < flxBens.Rows - 1 Then
                flxBens.Row = flxBens.Row + 1
            End If
        End If
        Text1.Text = ""
        Text1.Visible = False
